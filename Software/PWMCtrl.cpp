...
PWMCtrl::PWMCtrl(int pwmchip) {
    ...
    if (pwmchip < 0 || pwmchip >= NUM_OF_CHIPS)
        throw std::range_error("Couldn't create PWMCtrl as chip number is invalid.");

    // Execute Linux command that writes any modules containing the string "pwm" to a file temp.
    if (system(MODULECHECK.c_str()) == -1)
        throw std::runtime_error("PWM Module Check Error: Couldn't execute lsmod linux system command.");

    // Opens the file where the output of the command was piped to.
    tempFile.open(TEMPFILE.c_str());
    if (!tempFile.is_open())
        throw std::runtime_error("PWM Module Check Error: Couldn't open command output file.");

    // Checks that the file is not empty. If it is empty the PWM kernal module has not been installed.
    if (!getline(tempFile,line)) {
        tempFile.close();
        throw std::runtime_error("PWM Module Check Error: Couldn't find the neccessary kernal modules.");
    }
    tempFile.close();
    ...
    if(!exportFile.is_open())
        throw std::runtime_error("Couldn't carry out export as couldn't open PWM export linux system file.");
...
void PWMCtrl::configure(int period, int dutyCycle) {
    ...
    if (dutyCycle < 0 || dutyCycle > period)
        throw std::range_error("Invalid duty cycle. Must be between 0 and the period.");

    // Open the linux PWM period file.
    periodFile.open(periodFileName.c_str());
    if(!periodFile.is_open())
        throw std::runtime_error("Couldn't update period as couldn't open PWM period linux system file.");
    ...
    if(!dutyCycleFile.is_open())
        throw std::runtime_error("Couldn't update duty cycle as couldn't open PWM duty cycle linux system file.");
...
void PWMCtrl::enable(){
    ...
    if(!enableFile.is_open())
        throw std::runtime_error("Couldn't enable PWM as couldn't open PWM enable linux system file.");
...
void PWMCtrl::disable(){
    ...
    enableFile.open(enableFileName.c_str());
    if(!enableFile.is_open())
        throw std::runtime_error("Couldn't disable PWM as couldn't open PWM enable linux system file.");
